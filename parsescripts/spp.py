#!/usr/bin/python3
##############################################################
# Script name: St. Paul Pioneer Press (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "spp" # Short name for the publication
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        ##### Get Most Viewed ---- SPP IS SPECIAL AND DEVIATES FROM OTHER SCRIPTS DUE TO DATA COLLECTION ERROR
        if pubmv_external is not None:
            try:
                file_date = homepage.rstrip(".html")[-12:]
                mv_file = "/data/dissertation_scraper/html_src/sjm/sjm_poplist_%s.html" % file_date # DEBUG: No need for the hardcoded directory
        ...
        if layout == 1:
            try: # Main Story #1
                prominence_1 = [link.find("a", href=re.compile(pattern), class_="listingItemTitle") for link in document_soup.find("div", id="region3").find("div", class_="layout5FeatureItemBox")]
                prominence_linklist = []
            ...
            try: # Left Bar Stories
                prominence_2plus = [link.find("a", href=re.compile(pattern), class_="complexListingLink") for link in document_soup.find("div", id="region2").find("table", class_="complexListingBox").find_all("li")]
        ...
        try:
            document_soup_on_page.find("div", id="pop-feed").decompose() # Destroy the most-viewed list
        except:
