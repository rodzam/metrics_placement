#!/usr/bin/python3
##############################################################
# Script name: Star Tribune (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "mst" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(local|sports|business|politics|opinion|lifestyle|entertainment|obituaries|video|galleries|blogs|nation|world)/?(.+)?/(\d+).html?" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", class_="rightHeroColumn") is not None:
                layout = 2
        except:
            pass
        if layout is None:
            try:
                if document_soup.find("div", class_="additionalStory") is not None:
                    layout = 3
            except:
                pass
        if layout is None:
            try:
                if document_soup.find("div", class_="threeColContainer") is not None:
                    layout = 1
            except:
                pass
        if layout is None:
        ...
        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="mostReadStories").find_all("li")]
            mostviewed_linklist = []
        ...
        ### LAYOUT #1
        if layout == 1:
            try: # Carousel Stories -- although carousel is randomized in browser, it is static in the code, so we prioritize the code
                prominence_1to3 = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="carouselContent").find_all(re.compile("h1|h2|h3|h4"))]
                prominence_linklist = []
                for link in prominence_1to3:
                    try:
                        link = link.get("href")
                ...
                message = "Failed to get linklist for prominence 1 to 3 (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Right bar stories
                prominence_4plus = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="introContentContainer").find("ul", class_="moreHeadlines").find_all("li")]
                for link in prominence_4plus:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                    except:
                ...
                message = "Failed to get linklist for prominence 4plus (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ...
        if layout == 2:
            try: # Carousel Stories -- although carousel is randomized in browser, it is static in the code, so we prioritize the code
                ...
                message = "Failed to get linklist for prominence 1 to 3 (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # 'Hero' stories
                prominence_4plus = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="rightHeroColumn").find_all(re.compile("h1|h2|h3|h4"))]
                for link in prominence_4plus:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                ...
                message = "Failed to get linklist for prominence 4plus"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ...
        if layout == 3:
            try: # Carousel Stories -- although carousel is randomized in browser, it is static in the code, so we prioritize the code
                ...
                message = "Failed to get linklist for prominence 1 to 3 (layout 3)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Right 'Hero' story
                prominence_4plus = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="introContentContainer additionalStory").find_all("li")]
                for link in prominence_4plus:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                ...
                message = "Failed to get linklist for prominence 4plus (layout 3)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Latest news
                prominence_6plus = [link.find("a", href=link_pattern) for link in document_soup.find_all("div", class_="introContentContainer")[1].find("ul", class_="moreHeadlines").find_all("li")]
                ...
                message = "Failed to get linklist for prominence 6plus (layout 3)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ...
        try:
            document_soup_on_page.find("div", class_="mostReadCommentedBucket").decompose()
        except:
