#!/usr/bin/python3
##############################################################
# Script name: Kansas City Star (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "kcs" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(.+)/article(\d+).html?" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        ### LAYOUT #1
        if layout == 1:
            try: # Stories stacked nicely in the layout, just pick up the first five headlines
                prominence_1plus = [link.find("a", href=link_pattern) for link in document_soup.find("div", id="main-stage-top").find_all(re.compile("h1|h2|h3|h4"), class_="title")]
                prominence_linklist = []
