#!/usr/bin/python3
##############################################################
# Script name: Houston Chronicle (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "hch" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(.+)/(article|gallery|slideshow)/|/(.+)/(\d+)/(\d+)/" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", class_="redesign-three") is not None:
                layout = 1
        except:
            pass
        if layout is None:
        ...
        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="hst-mostpopular").find_all("li")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Featured Story
                prominence_1 = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="hst-centerpiecewide").find_all("h4")]
                prominence_linklist = []
                link = prominence_1[0]
                try:
                    link = link.get("href")
            ...
            try: # Left Bar Stories
                prominence_2plus = [link.find("a", href=link_pattern) for link in document_soup.find_all("div", class_="hst-headlinelistnarrow")[1].find_all("li")]
        ...
        try:
            document_soup_on_page.find("div", class_="hst-mostpopular").decompose()
        except:
