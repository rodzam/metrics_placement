#!/usr/bin/python3
##############################################################
# Script name: Fort Worth Star-Telegram (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "fws" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(\d+)/(\d+)/(\d+)/(\d+)/|/article(.+).html?" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", id="st_main_content").find("div", class_="grid_ab ") is not None:
                layout = 1
        except:
            pass
        if layout is None:
            try:
                if document_soup.find("div", id="main-stage-top") is not None:
                    layout = 2
            except:
                pass
        if layout is None:
        ...
        try:
            if layout == 1:
                mostviewed_1to5 = [link.find("a", href=link_pattern) for link in document_soup.find("div", id="popular").find("ul", class_="headline_stack").find_all("li")]
            elif layout == 2:
                mostviewed_1to5 = [link.find("a", href=link_pattern) for link in document_soup.find("section", id="trending").find_all("h1")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Featured story
                prominence_1 = [link.find("a", href=link_pattern) for link in document_soup.find("div", id="st_main_content").find("div", class_="featured_story").find_all(re.compile("(h1|h2|h3|h4)"))]
                prominence_linklist = []
            ...
            try: # Secondary stories
                prominence_2plus = [link.find("a", href=link_pattern) for link in document_soup.find("div", id="st_main_content").find("div", class_="secondary_stories").find_all(re.compile("(h1|h2|h3|h4)"))]
        ...
        if layout == 2:
            try: # Featured story
                prominence_1to4 = [link.find("a", href=link_pattern) for link in document_soup.find("div", id="main-stage-top").find("div", class_="section-piece-content").find_all(re.compile("(h1|h2|h3|h4)"))]
                prominence_linklist = []
                for link in prominence_1to4:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                    except:
                ...
                message = "Failed to get linklist for prominence 1 to 4 (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Right bar stories
                prominence_5plus = [link.find("a", href=link_pattern) for link in document_soup.find("div", id="main-stage-top").find("section", id="latest-news").find_all(re.compile("(h1|h2|h3|h4)"))]
                ...
                message = "Failed to get linklist for prominence 5plus (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue        
        ...
        try:
            if layout == 1:
                document_soup_on_page.find("div", id="popular_newest").decompose()
            elif layout == 2:
                document_soup_on_page.find("section", id="trending").decompose()
        except:
