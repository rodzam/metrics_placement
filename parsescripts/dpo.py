#!/usr/bin/python3
##############################################################
# Script name: The Denver Post (Parse Script)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "dpo" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/ci_(\d+)/?|(blogs|photos)(.+)\.com/(.+)/(.+)/|specialreports/ci_(.+)/?|thecannabist.com?/(\d+)/(\d+)/(\d+)/" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        try:
            mostviewed_1to5 = [link.find("a", href=link_pattern) for link in document_soup.find("div", id="pop-feed").find_all("li")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Main Story #1
                prominence_1 = [link.find("a", href=link_pattern, class_="listingItemTitle") for link in document_soup.find("div", id="region3").find("div", class_="layout5FeatureItemBox")]
                prominence_linklist = []
            ...
            try: # Left Bar Stories
                prominence_2plus = [link.find("a", href=link_pattern, class_="complexListingLink") for link in document_soup.find("div", id="region2").find("table", class_="complexListingBox").find_all("li")]
            ...
            try: # Supplemental Stories
                prominence_supp = [link.find("a", href=link_pattern, class_="complexListingLink") for link in document_soup.find("div", id="region3").find("div", class_="otherItemBox").find_all("li")]
                for link in prominence_supp:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                    except:
                ...
                message = "Failed to get linklist for prominence supp"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
