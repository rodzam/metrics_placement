#!/usr/bin/python3
##############################################################
# Script name: Washinton Post (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "wpo" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(.+)/(\d+)/(\d+)/(\d+)/|/wp-srv/special/" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", class_="wp-column margin-right six") is not None:
                layout = 2
        except:
            pass
        if layout is None:
            try:
                if document_soup.find("div", class_="wp-column ten margin-right main-content") is not None:
                    layout = 1
            except:
                pass
        if layout is None:
        ...
        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="post_most").find_all("li")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Get left bar stories
                prominence_leftbar = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="wp-column margin-right four-mini").find_all("h2")]
                ...
                message = "Failed to get linklist for prominence leftbar"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Get right bar stories
                prominence_rightbar = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="wp-column six margin-left").find_all("h2")]
                ...
                message = "Failed to get linklist for prominence rightbar"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Get remaining main headlines
                prominence_supplemental = [link.find("h2").find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="wp-column margin-right five grid2_col_left").find_all("div", class_="module s1 img-border ")]
                ...
                message = "Failed to get linklist for prominence supplemental"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            ...
            try: # Now, order the prominence linklist. We favor the top-left because it has the biggest headline, then the right because it has a featured image, then the remainder of the left bar, then the remainder of the right bar, followed by any supplemental stories
                try:
                    ...
                    prominence_linklist_tmp = prominence_leftbar_linklist + prominence_rightbar_linklist + prominence_supplemental_linklist # Because we check for duplicates, we can follow this
                prominence_linklist = []
                ...
                message = "Failed to produce an ordered prominence linklist"
                seriousness = 2
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ...
        if layout == 2:
            try: # Get left bar stories
                prominence_leftbar = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="wp-column margin-right six").find_all("h2")]
                prominence_leftbar_linklist = []
                for link in prominence_leftbar:
                    try:
                        link = link.get("href")
            ...
            try: # Get right bar stories
                prominence_rightbar = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="wp-column four-mini margin-left").find_all("h2")]
                prominence_rightbar_linklist = []
                for link in prominence_rightbar:
                    try:
                        link = link.get("href")
        ...
        try:
            document_soup_on_page.find("div", id="post_most").decompose()
        except:
