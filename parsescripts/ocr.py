#!/usr/bin/python3
##############################################################
# Script name: Register (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "ocr" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/articles/(.+).html?|/(.+)-(\d+)-(.+).html?" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", id="rotatorAbox") is not None:
                layout = 1
        except:
            pass
        if layout is None:
        ...
        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("li", class_="most-popular").find_all("li")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Lead story
                prominence_1 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="lead-story").find_all(re.compile("h1|h2|h3|h4"))]
                prominence_linklist_lead = []
                for link in prominence_1:
                    try:
                        link = link.get("href")
                        prominence_linklist_lead = parserfunctions.linklist_actions(link, prominence_linklist_lead)
                    except:
                        pass
            except:
            ...
            try: # Lead Rail
                prominence_2plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="lead-rail").find_all(re.compile("h1|h2|h3|h4"))]
                prominence_linklist_leadrail = []
                for link in prominence_2plus:
                    try:
                        link = link.get("href")
                        prominence_linklist_leadrail = parserfunctions.linklist_actions(link, prominence_linklist_leadrail)
                    except:
                        pass
            except:
            ...
            try: # Supplemental stories
                prominence_6plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("ul", id="SectionList").find_all(re.compile("li"))] # Although broad, the pattern keeps it sane
                prominence_linklist_supp = []
                for link in prominence_6plus:
                    try:
                        link = link.get("href")
                        prominence_linklist_supp = parserfunctions.linklist_actions(link, prominence_linklist_supp)
                    except:
                        pass
            except:
            ...
            try: # Now, order the prominence linklist. We favor the top-left because it has the biggest headline, then the right because it has a featured image, then the remainder of the left bar, then the remainder of the right bar, followed by any supplemental stories
                try:
                    prominence_linklist_tmp = [prominence_linklist_leadrail[0]] + prominence_linklist_lead + prominence_linklist_leadrail[1:] + prominence_linklist_supp # Because we check for duplicates, we can follow this
                except:
                    prominence_linklist_tmp = prominence_linklist_lead + prominence_linklist_leadrail + prominence_linklist_supp # Because we check for duplicates, we can follow this
                prominence_linklist = []
        ...
        try:
            document_soup_on_page.find("li", class_="most-popular").decompose()
        except:
