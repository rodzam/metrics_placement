...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        if layout == 1:
            try: # Lead story
                prominence_1 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="main-content").find_all("div", class_="headline")]
                prominence_linklist = []
            ...
            try: # Go with the editor's picks thereafter
                prominence_2plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="main-content").find("div", text="EDITOR'S PICKS").parent.find_all("li")]
        ...
        ### LAYOUT #2
        if layout == 2:
            try: # This is a seldom-used, but fairly static layout
                prominence_1plus = [link for link in document_soup.find("div", id="mainGrid").find_all("a", href=re.compile(pattern))]
                prominence_linklist = []
                ...
                message = "Failed to get linklist for prominence 1plus (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Reorder links to match prominence
                prominence_linklist = [prominence_linklist[2]] + [prominence_linklist[0]] + [prominence_linklist[1]] + prominence_linklist[3:]
            except:
                message = "Failed to reorder link list for prominence (layout 2)"
                seriousness = 2
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
