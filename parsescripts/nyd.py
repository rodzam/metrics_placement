#!/usr/bin/python3
##############################################################
# Script name: Daily News (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", class_="teaser hero") is not None:
                layout = 1
        except:
            pass
        if layout is None:
            try:
                if document_soup.find("div", id="content_container") is not None:
                    layout = 2
            except:
                pass
        if layout is None:
        ...
        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="list-7-1353782").find_all("li")]
            mostviewed_linklist = []
        ...
        ### LAYOUT #1
        if layout == 1:
            try: # Teaser Hero story
                prominence_1plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="teaser hero").find_all(re.compile("h1|h2|h3|h4"))]
                prominence_linklist = []
            ...
            try: # Main bar stories
                prominence_2plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="content_container").find_all(re.compile("h1|h2|h3|h4"))]
        ...
        try:
            document_soup_on_page.find("div", id="most-read-content").decompose()
        except:
