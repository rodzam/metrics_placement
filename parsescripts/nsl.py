#!/usr/bin/python3
##############################################################
# Script name: The Star-Ledger (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "nsl" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(\d+)/(\d+)/(.+).html?|/news/article/|nj.com/a-teachers-betrayal" # Pattern for defining what is and what is not a news item; Special regex for a special feature that came up on 11/09
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        if layout is None:
            try:
                if document_soup.find("div", id="three-over-two") is not None:
                    layout = 1 # Because of our approach, we can use the same layout for all of these
            except:
                pass
        if layout is None:
            try:
                if document_soup.find("div", class_="three-col-promo") is not None:
                    layout = 1 # Because of our approach, we can use the same layout for all of these
            except:
                pass
        if layout is None:
            try:
                if document_soup.find("div", class_="big-story clearfix topic-one") is not None:
                    layout = 1 # Because of our approach, we can use the same layout for all of these
            except:
                pass
        if layout is None:
