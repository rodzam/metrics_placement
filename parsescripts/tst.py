#!/usr/bin/python3
##############################################################
# Script name: Seattle Times (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "tst" # Short name for the publication
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        if layout is None:
            try:
                if document_soup.find("div", class_="co_4colvisual") is not None:
                    layout = 2
            except:
                pass
        if layout is None:
        ...
        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="mostreademailed").find_all("li")] # ID is most e-mailed, but page states most read
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Lead story
                prominence_1 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_=re.compile("co_3colvisual|co_2colvisual|co_2_5colvisual")).find_all(re.compile("h1|h2|h3|h4"))]
                prominence_linklist = []
                ...
                message = "Failed to get linklist for prominence 1 (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Right bar top stories
                prominence_2plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="co_mainstories").find_all(re.compile("h1|h2|h3|h4|h5"))]
                ...
                message = "Failed to get linklist for prominence 2plus (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Right bar lesser stories -- need this distinction else it picks up related items
                prominence_3plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="co_mainstories").find("ul", class_="").find_all("li")] # Although broad, the pattern keeps it sane
                for link in prominence_3plus:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                    except:
                ...
                message = "Failed to get linklist for prominence 3plus (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ...
        if layout == 2:
            try: # Lead story
                prominence_1 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_=re.compile("co_4colvisual")).find_all(re.compile("h1|h2|h3|h4"))]
                prominence_linklist = []
                for link in prominence_1:
                    try:
                        link = link.get("href")
                ...
                message = "Failed to get linklist for prominence 1 (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Bottom-left stories
                prominence_2plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="co_4colvisual_stories").find_all(re.compile("h1|h2|h3|h4|h5"))]
                for link in prominence_2plus:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                ...
                message = "Failed to get linklist for prominence 2plus (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Bottom-right stories -- need this distinction else it picks up related items
                prominence_3plus = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", class_="co_4colvisual_stories").find("div", class_="columns34").find_all(re.compile("li"))] # Although broad, the pattern keeps it sane
                for link in prominence_3plus:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                ...
                message = "Failed to get linklist for prominence 3plus"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ### For all layouts
        ...
        try:
            document_soup_on_page.find("div", id="mostreademailed").decompose()
        except:
