#!/usr/bin/python3
##############################################################
# Script name: St. Louis Post-Dispatch (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "slp" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(.+)/(article|html|collection)_(.+).html?" # Pattern for defining what is and what is not a news item
pub_tz = "US/Central" # Timezone the publication is in
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        prominence_left_linklist, prominence_right_linklist = [None, None] # Publication-specific
        ...
        layout = None
        try:
            if len(list(document_soup.find("div", id="tncms-region-center-one-front").descendants)) >= 8:
                layout = 2
        except:
            pass
        if layout is None:
            try:
                if len(list(document_soup.find("div", id="tncms-region-center-two-front").descendants)) >= 8:
                    layout = 1
            except:
                pass
        if layout is None:
            print("Failed to detect the layout for document %s" % (homepage))
            continue

        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="most-popular-tabs").find("div", class_="tab").find_all("li")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Left Rail
                prominence_left = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="tncms-region-center-two-front").find_all(re.compile("h1|h2|h3"))]
                prominence_left_linklist = []
                for link in prominence_left:
                    try:
                        link = link.get("href")
                        prominence_left_linklist = parserfunctions.linklist_actions(link, prominence_left_linklist)
                    except:
                        pass
            except:
                message = "Failed to get linklist for prominence left (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Right Rail
                prominence_right = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="tncms-region-center-three-front").find_all(re.compile("h1|h2|h3"))]
                prominence_right_linklist = []
                for link in prominence_right:
                    try:
                        link = link.get("href")
                        prominence_right_linklist = parserfunctions.linklist_actions(link, prominence_right_linklist)
                    except:
                        pass
            except:
                message = "Failed to get linklist for prominence 1plus (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Order the stories from the two rails
                prominence_linklist_tmp = [prominence_left_linklist[0]] + prominence_right_linklist + prominence_left_linklist[1:]
                prominence_linklist = []
                ...
                message = "Failed to merge the linklists for prominence (layout 1)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ...
        if layout == 2:
            try: # Top Box
                prominence_top = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="tncms-region-center-one-front").find_all(re.compile("h1|h2|h3"))]
                prominence_linklist = []
                for link in prominence_top:
                    try:
                        link = link.get("href")
                ...
                message = "Failed to get linklist for prominence top (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
            
            try: # Bottom-left Rail
                prominence_bleft = [link.find("a", href=re.compile(pattern)) for link in document_soup.find("div", id="tncms-region-center-two-front").find_all(re.compile("h1|h2|h3"))]
                for link in prominence_bleft:
                    try:
                        link = link.get("href")
                        prominence_linklist = parserfunctions.linklist_actions(link, prominence_linklist)
                    except:
                ...
                message = "Failed to get linklist for prominence prominence bleft (layout 2)"
                seriousness = 3
                parserfunctions.error_log_entry(cur, conn, mysql_log_name, curr_time, pubshort, homepage, seriousness, message)
                #continue
        ...
        try:
            document_soup_on_page.find("div", id="most-popular-tabs").decompose()
        except:
