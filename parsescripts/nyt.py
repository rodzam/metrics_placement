#!/usr/bin/python3
##############################################################
# Script name: New York Times (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "nyt" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(\d+)/(\d+)/(\d+)/|/video/(.+).html?" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", class_="span-ab-layout layout") is not None:
                layout = 1
        except:
            pass
        if layout is None:
        ...
        ##### Get Most Viewed
        try:
            mostviewed_1to5 = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="tab-content most-viewed").find_all("li")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Lead story
                prominence_1 = [link.find("a", href=link_pattern) for link in document_soup.find("main", id="main").find_all("article", class_="story theme-summary lede")]
                prominence_linklist = []
            ...
            try: # NYT orders stories nicely with a specific class
                prominence_2plus = [link.find("a", href=link_pattern) for link in document_soup.find("main", id="main").find_all("article", class_="story theme-summary")]
        ...
        try:
            document_soup_on_page.find("section", id="recommendations").decompose()
        except:
