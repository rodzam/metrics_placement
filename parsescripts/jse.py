#!/usr/bin/python3
##############################################################
# Script name: Milwaukee Journal Sentinel (Page Parser)
# Version: 2.0
# By: Rodrigo Zamith
...
pubshort = "jse" # Short name for the publication
pubmv_external = None # Do we need to get the list of most viewed items from another page? (None = No, anything else = Yes)
pattern = "/(.+)/(.+)?-?(\d+).html?" # Pattern for defining what is and what is not a news item
...
if process_desktop is not None:
    ...
    for homepage in file_list:
        ...
        layout = None
        try:
            if document_soup.find("div", class_="lead_story") is not None:
                layout = 1
        except:
            pass
        if layout is None:
        ...
        ### Get Most Viewed #1
        try:
            mostviewed_1to5 = [link.find("a", href=link_pattern) for link in document_soup.find("ul", id="popularList1_List1").find_all("li")]
            mostviewed_linklist = []
        ...
        if layout == 1:
            try: # Lead Stories
                prominence_1to3 = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="lead_story").find_all(re.compile("h1|h2|h3|h4"))]
                prominence_linklist = []
            ...
            try: # Featured Stories
                prominence_4plus = [link.find("a", href=link_pattern) for link in document_soup.find("div", class_="featured_stories").find_all(re.compile("h1|h2|h3|h4"))]
        ...
        try:
            document_soup_on_page.find("div", id="popularList1").decompose()
        except:
